...
**Version**: 0.2.1 | **Package**: `any2md` | **Entry point**: `any2md.cli:app`
...
│   ├── cli.py           # Unified entry point — auto-detect + subcommands (299 lines)
│   ├── common.py        # Shared: build_frontmatter(), setup_logging(), JSON mode (288 lines)
...
│   └── repo.py          # Git repositories via repomix (npm wrapper) (~5.4K)
├── tests/
...
│   ├── test_db.py, test_sub.py, test_nb.py, test_eml.py
│   ├── test_org.py, test_tex.py, test_man.py, test_repo.py
│   ├── test_cli.py, test_common.py
...
**Total**: ~9,500 lines of source code across 20 modules. 808 tests (8,810 lines).
...
| `pymupdf4llm>=0.2.0` | PDF text extraction | `[pdf]` | ❌ |
...
| `repo.py` | `process_repo_dir()` | repomix (npm) | Requires `repomix` installed globally |
